    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      {activePost ? (
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <article className="glass p-8 md:p-12 rounded-2xl border border-border/80">
            {/* Meta */}
            <div className="flex flex-wrap gap-4 text-xs text-muted font-body mb-6 items-center">
              <span className="bg-primary/10 text-primary border border-primary/20 px-2.5 py-0.5 rounded-full font-semibold uppercase tracking-wider font-display">
                {activePost.category}
              </span>
            ...
            {/* Title */}
            <h1 className="text-3xl sm:text-4xl font-black font-display text-text leading-tight mb-6">
              {activePost.title}
            </h1>

            {/* Author */}
            <div className="flex items-center gap-2 text-sm text-text font-body mb-8 border-b border-border/40 pb-4">
              <User size={16} className="text-primary" />
              <span>By {activePost.author}</span>
            </div>

            {/* Body */}
            <div className="font-body text-base text-muted leading-relaxed space-y-6 whitespace-pre-line">
              {activePost.content}
            </div>
          </article>
        </div>
        ...
        <div>
          <div className="text-center mb-16">
            ...
            <p className="text-muted font-body mt-4 max-w-md mx-auto leading-relaxed">
              IT guides, cybersecurity audits, backup strategies, and hardware reviews written by our certified technicians.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {POSTS.map((post) => (
              ...
              >
                {/* Meta */}
                <div className="flex items-center justify-between text-xs text-muted font-body mb-4">
                  <span className="text-primary font-semibold uppercase tracking-wider font-display">
                    {post.category}
                  </span>
                  <span className="flex items-center gap-1">
                    <Calendar size={12} />
                ...
                {/* Title */}
                <h3 className="text-xl font-bold font-display text-text mb-3 leading-snug">
                  {post.title}
                </h3>
